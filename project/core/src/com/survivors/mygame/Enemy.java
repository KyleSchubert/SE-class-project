...
public class Enemy extends Mobile {
    ...
    private final EnemyData enemyData; // This exclusively contains the information that will not change about the specific enemy.
     ...
     * the frames and frame delays of the specific enemy's current state (like STANDING or MOVING) can be found.
    ...
    /**
     * @param enemyTypeName The type of the enemy that will be created. This is an enum: EnemyData.EnemyTypeName
     ...
     */
    public Enemy(EnemyTypeName enemyTypeName, int x, int y) {
        this.enemyData = new EnemyData(enemyTypeName);
        this.x = x;
        this.y = y;
        this.setState(EnemyState.STANDING);
    }

    /**
     * @param state The state of the enemy that you want it to be changed to. Ex: Enemy.EnemyState.MOVING
     */
    public void setState(EnemyState state) {
        if (state == EnemyState.ATTACKING && !this.enemyData.canAttack) {
            return;
        } else {
            this.state = state;
        }
    ...
    private void prepareFrameStartAndEndIndex() {
        switch (this.state) {
            case DYING:
                this.stateFrameStartIndex = this.enemyData.dyingAnimationStartFrameIndex;
                this.stateFrameEndIndex = this.enemyData.dyingAnimationEndFrameIndex;
                break;
            case MOVING:
                this.stateFrameStartIndex = this.enemyData.movingAnimationStartFrameIndex;
                this.stateFrameEndIndex = this.enemyData.movingAnimationEndFrameIndex;
                break;
            case STANDING:
                this.stateFrameStartIndex = this.enemyData.standingAnimationStartFrameIndex;
                this.stateFrameEndIndex = this.enemyData.standingAnimationEndFrameIndex;
                break;
            case ATTACKING:
                this.stateFrameStartIndex = this.enemyData.attackingAnimationStartFrameIndex;
                this.stateFrameEndIndex = this.enemyData.attackingAnimationEndFrameIndex;
                break;
        }
    }
     ...
     * You can change the animation by changing the Enemy's state with .setState(EnemyState state)
     *
    ...
    public void animate(SpriteBatch batch, float elapsedTime) {
        ...
        if (this.frame == this.stateFrameEndIndex) {
            if (this.frameTime > this.enemyData.allAnimationFrameDelays.get(this.stateFrameStartIndex)) {
                this.frame = this.stateFrameStartIndex;
                this.frameTime -= this.enemyData.allAnimationFrameDelays.get(this.stateFrameStartIndex);
            }
        } else {
            if (this.frameTime > this.enemyData.allAnimationFrameDelays.get(this.frame + 1)) {
                this.frameTime -= this.enemyData.allAnimationFrameDelays.get(++this.frame);
            }
        }

        batch.draw(this.enemyData.allAnimationFrames[this.frame],
                this.x - this.enemyData.originX, this.y - (this.enemyData.dimensionY - this.enemyData.originY),
                this.enemyData.dimensionX, this.enemyData.dimensionY);
    }


    /**
     * EnemyData:
     * This exclusively contains the information that will not change about the specific enemy.
     */
    static class EnemyData {
        private TextureRegion[] allAnimationFrames;
        private ArrayList<Float> allAnimationFrameDelays = new ArrayList<>();
         ...
         * take it and compare it against all the valid enemy types
         * which will then assign specific values to variables in this class like
         * how big the enemy is, or its corresponding hit box, or how much Max HP it has, etc.
         *
         * @param enemyTypeName The word for the type of the enemy to be created.
         */
        public EnemyData(EnemyTypeName enemyTypeName) {
            switch (enemyTypeName) {
                ...
                case SMALL_PENGUIN:
                    ...
                    break;
                default:
                    System.out.println("Why was an enemy almost generated with no matching type name? enemyTypeName:  " + enemyTypeName);
                    return;
            }
            prepareSpritesheets();
